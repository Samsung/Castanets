...
public class MotionEventBuilder {
   ...
   */
  public MotionEventBuilder setPointer(float x, float y, int id) {
    PointerProperties pointerProperties = new PointerProperties();
    pointerProperties.id = id;
    PointerCoords pointerCoords = new PointerCoords();
    pointerCoords.x = x;
    pointerCoords.y = y;
    return setPointer(pointerProperties, pointerCoords, id);
  }
   ...
   */
  public MotionEventBuilder setPointer(PointerProperties pointerProperties, PointerCoords pointerCoords, int id) {
    for(int i = 0 ; i < pointerPropertiesList.size(); i++){
      if(id == pointerPropertiesList.get(i).id){
        pointerCoordsList.get(i).x = pointerCoords.x;
        pointerCoordsList.get(i).y = pointerCoords.y;
        return this;
      }
    }
  ...
  /**
   * Remove Pointer with id from Pointer list
   */
  public void removePointer(int id) {
    for(int i = 0 ; i < pointerPropertiesList.size(); i++){
      if(id == pointerPropertiesList.get(i).id){
        pointerPropertiesList.remove(i);
        pointerCoordsList.remove(i);
      }
    }
  }
