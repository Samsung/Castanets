...
g_numCores = 4
g_maxNetDelay = 5001 # max 5 sec delay
g_netBandwidth = 104857600 # 100Mbps as default. Modification needed after testing

g_benchSiteFile = 'top-200.csv'
...
def writeLoadingTime(url, latency, packetDropRate, bigcoreFreq, littlecoreFreq, numBigCores, numLittleCores, cpuUtil, csv_writer):
    ...
    csv_writer.writerow([latency, bandwidth, bigcoreFreq, littlecoreFreq, numBigCores,numLittleCores, cpuUtil, str(loadingTime.split()[0])])
...
def _main():
    ...
    csv_writer.writerow(["latency(ms)", "bandwidth(bps)", "bigcoreFreq", "littlecoreFreq", "numBigCores","numLittleCores", "cpu Utilization", "loadingTime(ms)"])
    prefix = "http://www."
            ...
            if bfreq > lfreq: # big freq should be larger than little freq
               for numLittleCore in range(0, g_numCores, 1): #numCores range between 0 ~ numCore-1
                   for cpuUtil in range(0, 80, 20): # for CPU usage 0% to 90%
                       for netDelay in range(0, g_maxNetDelay, 1000): # for network delay 0 to 5sec
                           for netPacketDropRate in range(0, 80, 20): # 
