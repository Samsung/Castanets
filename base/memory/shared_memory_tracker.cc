...
std::unique_ptr<UnknownMemorySyncer>
SharedMemoryTracker::TakeUnknownMemory(int fd) {
...
CastanetsMemorySyncer *
SharedMemoryTracker::GetSyncer(const UnguessableToken &guid) {
  {
    AutoLock syncer_hold(syncer_lock_);
    auto syncer = memory_syncers_.find(guid);
    if (syncer != memory_syncers_.end())
