  ...
  if (!connection_params.is_async()) {
    ...
    if (base::Castanets::IsEnabled()) {
      ...
      if (!connection_params.endpoint().is_valid() &&
        ...
        // and the broker was unable to negotiate a NodeChannel pipe. In this
        // case we can cancel our connection to our inviter.
...
void NodeController::SendBrokerClientInvitationOnIOThread(
...
#endif
  {
    //  ConnectionParams node_connection_params;
    if (!connection_params.is_async()) {
      // Sync connections usurp the passed endpoint and use it for the sync
      // broker channel. A new channel is created here for the NodeChannel and
      // sent over a sync broker message to the client.
      PlatformChannel node_channel;
      node_connection_params =
          ConnectionParams(node_channel.TakeLocalEndpoint());
