...
void BrokerCastanets::SendSyncEvent(
  ...
  SyncSharedBufferImpl(mapping_info->guid(),
                       ...
                       BrokerCompressionMode::ZLIB, write_lock);
}
...
bool BrokerCastanets::SyncSharedBuffer(
  ...
  SyncSharedBufferImpl(mapping.guid(), static_cast<uint8_t*>(mapping.memory()),
                       offset, sync_size, mapping.mapped_size(),
                       BrokerCompressionMode::ZLIB);
  return true;
}

bool BrokerCastanets::SyncSharedBuffer2d(
  ...
  SyncSharedBufferImpl2d(guid, static_cast<uint8_t*>(mapping->GetMemory()),
                         mapping->mapped_size(), width, height, bytes_per_pixel,
                         offset, stride, compression_mode);
  return true;
}

void BrokerCastanets::SyncSharedBufferImpl(
    const base::UnguessableToken& guid,
    uint8_t* memory,
    size_t offset,
    size_t sync_size,
...
static size_t WebPEncodeLosslessRGBAExactly(const uint8_t* rgba,
                                            int width,
                                            int height,
                                            int stride,
                                            uint8_t** output) {
...
void BrokerCastanets::SyncSharedBufferImpl2d(
    const base::UnguessableToken& guid,
    uint8_t* memory,
    size_t mapped_size,
    size_t width,
  ...
  switch (compression_mode) {
    ...
    case BrokerCompressionMode::WEBP: {
      ...
      VLOG(2) << "WEBP Compression: Raw Size: " << sync_size
              << ", Compressed Size: " << size << ", Buffer width: " << width
    ...
    case BrokerCompressionMode::NONE: {
      if (offset == 0 && stride == 0) {
        ...
        while (put_bytes != sync_size) {
          memcpy(static_cast<uint8_t*>(&bytes[0]) + put_bytes,
                 memory + put_offset, buffer_stride);
          put_offset += stride;
...
void BrokerCastanets::OnBufferSync2d(uint64_t guid_high,
  ...
  scoped_refptr<base::CastanetsMemoryMapping> mapping =
      base::SharedMemoryTracker::GetInstance()->FindMappedMemory(guid);
  uint8_t* ptr =
      mapping ? static_cast<uint8_t*>(mapping->GetMemory()) : nullptr;
  if (!ptr) {
    base::SharedMemoryCreateOptions options;
    options.size = buffer_bytes;
    base::subtle::PlatformSharedMemoryRegion handle =
      ...
      if (offset == 0 && stride == 0) {
        ...
        while (put_bytes != sync_bytes) {
          memcpy(static_cast<uint8_t*>(ptr + put_offset),
                 static_cast<const uint8_t*>(uncompressed_data) + put_bytes,
                 width);
          put_offset += stride;
          put_bytes += width;
        }
