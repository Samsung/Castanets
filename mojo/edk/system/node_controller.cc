...
void NodeController::SendBrokerClientInvitation(
...

  {
    ...
    for (auto& entry : attached_ports) {
      auto result = port_map.insert(std::make_pair(entry.first, entry.second));
  ...
  io_task_runner_->PostTask(
      FROM_HERE,
      base::Bind(&NodeController::SendBrokerClientInvitationOnIOThread,
                 base::Unretained(this), target_process,
                 base::Passed(&connection_params), temporary_node_name,
...
void NodeController::AcceptBrokerClientInvitation(
  ...
  io_task_runner_->PostTask(
      FROM_HERE,
      base::Bind(&NodeController::AcceptBrokerClientInvitationOnIOThread,
...
#if defined(CHROMIE)
  ScopedPlatformHandle server_handle = mojo::edk::CreateTCPServerHandle(
      mojo::edk::kChromieBrokerPort);
...

  {
    ...
    if (it != reserved_ports_.end()) {
      ...
      auto result = reserved_ports_.insert(
          std::make_pair(child_name, std::move(it->second)));
      DCHECK(result.second);
      reserved_ports_.erase(it);
    }
  }
